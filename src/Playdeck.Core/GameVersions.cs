...
public sealed class GameVersions:IDisposable {
 readonly HttpClient http;readonly SemaphoreSlim gate=new(1,1);
 public GameVersions(HttpMessageHandler? handler=null){http=handler==null?new HttpClient():new HttpClient(handler);http.Timeout=TimeSpan.FromSeconds(12);http.MaxResponseContentBufferSize=2*1024*1024;http.DefaultRequestHeaders.UserAgent.ParseAdd("Playdeck/6.1 (game-version-check)");}
 ...
 public static VersionRecord ReadLocal(Game game,IEnumerable<string>? steamRoots=null){
  var v=JsonSerializer.Deserialize<VersionRecord>(JsonSerializer.Serialize(game.VersionInfo,Store.Json),Store.Json)!;v.LocalAvailable=false;v.LocalPath="";v.ObservedAt=DateTimeOffset.UtcNow;v.Installed="";v.InstalledBuild="";v.LatestBuild="";v.RemoteCheckedAt=null;v.NeedsUpdate=false;v.InstallationReady=true;v.AppId=game.SteamId;v.Branch="public";v.Detail="";
  if(v.Mode=="Manual"){v.Installed=v.ManualVersion.Trim();v.LocalAvailable=v.Installed.Length>0;v.Source="Manual entry";v.Status=v.Installed.Length>0?"Recorded manually":"Enter an installed version";if(v.Installed.Length>0&&v.ManualLatest.Length>0){int? result=CompareLabels(v.Installed,v.ManualLatest);v.NeedsUpdate=result>0;v.Status=result==0?"Matches your latest version":result>0?"Newer version noted":result<0?"Installed version is newer":"Different labels · cannot order";v.Detail="Compared only with the latest version you entered, not a Steam build ID.";}return v;}
 ...
 static IEnumerable<string> SteamRoots(){var result=new HashSet<string>(StringComparer.OrdinalIgnoreCase);try{using var key=Registry.CurrentUser.OpenSubKey(@"Software\Valve\Steam");if(key?.GetValue("SteamPath") is string path)result.Add(path);}catch{}foreach(var root in result.ToArray()){try{var folders=VdfNode.Parse(ReadText(Path.Combine(root,"steamapps","libraryfolders.vdf"))).Node("libraryfolders");if(folders!=null)foreach(var entry in folders.Children.Values){string p=entry.Text("path");if(p.Length>0)result.Add(p);}}catch{}}return result;}
