...
public sealed record VersionBadge(string Text,string Color,bool NeedsHelp) {
 ...
 public static VersionBadge For(VersionRecord v,DateTimeOffset? now=null){
  ...
  if(v.ReleaseSourceKey.Length>0&&v.ManualLatest.Length==0){
   bool recent=v.ReleaseCheckedAt.HasValue&&time>=v.ReleaseCheckedAt.Value&&time-v.ReleaseCheckedAt.Value<TimeSpan.FromHours(24);
   if(v.LocalAvailable&&v.ConfirmedReleaseSource==v.ReleaseSourceKey){
    if(v.NeedsUpdate)return new("NEWER RELEASE · "+v.Installed+(recent?"":" · cached"),"#F4D35D",false);
    if(v.MatchesLatest&&recent&&v.ReleaseError.Length==0)return new("MATCHES RELEASE · "+v.Installed,"#B6EC78",false);
   }
   return new(v.Installed.Length==0?"SET VERSION":v.Status+" · "+v.Installed,"#C1BBCB",true);
  }
