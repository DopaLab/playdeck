...
internal static class Program {
 ...
 static async Task<int> Main(string[] args){
  ...
  try{
   ...
   if(args.Length>0){string tracker=Path.GetFullPath(args[0]);await TrackerTest(tracker,false);await TrackerTest(tracker,true);await TrackerTest(tracker,false,true);}
