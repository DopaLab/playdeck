...
public sealed class GameSurface:FrameworkElement {
 readonly string name,detail;readonly ImageSource cover;readonly ImageSource? centerIcon,localIcon;readonly bool compact,showBadge;readonly VersionBadge badge;readonly string versionLabel;
 FormattedText? titleText,detailText,badgeText;double textWidth,textDpi;bool showPlay;
 ...
 public GameSurface(string name,string detail,ImageSource cover,ImageSource? centerIcon,ImageSource? localIcon,bool pinned,bool compact,VersionRecord version,Game? game=null,string mode="Version"){this.name=name;this.detail=detail;this.cover=cover;this.centerIcon=centerIcon;this.localIcon=localIcon;this.compact=compact;showBadge=mode!="Off"&&game?.IsTool!=true;badge=mode=="Update date"&&game!=null?UpdateDates.Badge(game):VersionBadge.For(version);versionLabel=mode=="Update date"&&game!=null?UpdateDates.Label(game):VersionBadge.CompactLabel(version);SnapsToDevicePixels=true;}
 double badgeWidth,badgeDpi;
 public Action? OpenVersion;
 public bool IsVersionHit(Point point)=>showBadge&&ActualWidth>64&&new Rect(11,11,VersionChipWidth(ActualWidth),20).Contains(point);
 ...
 sealed class VersionPeer(GameSurface owner):System.Windows.Automation.Peers.FrameworkElementAutomationPeer(owner),System.Windows.Automation.Provider.IInvokeProvider {
  protected override string GetNameCore()=>"Update details for "+owner.name+" · "+owner.badge.Text;
  ...
  protected override Rect GetBoundingRectangleCore(){if(!owner.IsVisible||!owner.showBadge)return Rect.Empty;var dpi=VisualTreeHelper.GetDpi(owner);return new Rect(owner.PointToScreen(new Point(11,11)),new Size(owner.VersionChipWidth(owner.ActualWidth)*dpi.DpiScaleX,20*dpi.DpiScaleY));}
 ...
 protected override void OnRender(DrawingContext dc){double w=ActualWidth,h=ActualHeight,art=h-71;if(w<=0||art<=0)return;dc.PushClip(new RectangleGeometry(new Rect(0,0,w,h),15,15));FillImage(dc,cover,new Rect(0,0,w,art));
  ...
  VersionChipWidth(w);
  if(showBadge){double badgeY=11;dc.DrawRoundedRectangle(BadgeTexture,new Pen(ChartInk.Brush(badge.Color),1),new Rect(11,badgeY,badgeWidth,20),10,10);dc.DrawText(badgeText!,new Point(11+(badgeWidth-badgeText!.Width)/2,badgeY+(20-badgeText.Height)/2));}
